...
describe('Enhanced Agent Loading (Phase 2) - With Mocks', () => {
  ...
  describe('Enhanced Loading with Retry', () => {
    ...
    it('should retry loading on transient failures', async () => {
      ...
      const loaded = await loadAgentFromDiskWithRetry(testWorldId, 'retry-agent-3', options);
    ...
    it('should return null after exhausting retries', async () => {
      ...
      const loaded = await loadAgentFromDiskWithRetry(testWorldId, 'retry-agent-4', options);
  ...
  describe('Batch Loading', () => {
    it('should load multiple agents in batch', async () => {
      ...
      const result: BatchLoadResult = await loadAllAgentsFromDiskBatch(testWorldId);

      expect(result.successful).toHaveLength(2);
      expect(result.failed).toHaveLength(0);
      expect(result.successful.map(a => a.id)).toContain('batch-agent-1');
      expect(result.successful.map(a => a.id)).toContain('batch-agent-2');
    });
