 ...
 * - Executes instruction-referenced scripts only after HITL approval
...
describe('core/load-skill-tool', () => {
  ...
  it('skips script execution when HITL approval is declined', async () => {
    mockedGetSkill.mockReturnValue({
      skill_id: 'pdf-extract',
      description: 'Extract PDF content',
      hash: 'e99a18ad',
    ...
    expect(result).toContain('User declined skill script execution');
  });
});
