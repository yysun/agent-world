...
describe('First Mention Only - Agent Response Logic', () => {
  ...
  describe('First Mention Only Response Logic', () => {
    it('should only respond if agent is first mention - a1 responds, a2 does not', async () => {
      ...
      const a1ShouldRespond = await shouldRespondToMessage(agent1Config, messageData);
      expect(a1ShouldRespond).toBe(true);

      // a2 should NOT respond (second mention)
      const a2ShouldRespond = await shouldRespondToMessage(agent2Config, messageData);
      expect(a2ShouldRespond).toBe(false);
    });

    it('should handle multiple mentions where first is unknown agent', async () => {
      const messageData: MessageData = {
        name: 'test-message',
    ...
    it('should handle multiple mentions - only first agent responds', async () => {
      const messageData: MessageData = {
        name: 'test-message',
    ...
    it('should still treat no mentions as public message - all agents respond', async () => {
      const messageData: MessageData = {
        name: 'test-message',
    ...
    it('should handle case insensitive first mention matching', async () => {
      const messageData: MessageData = {
        name: 'test-message',
    ...
    it('should handle agent-to-agent messages with first mention only', async () => {
      const messageData: MessageData = {
        name: 'test-message',
    ...
    it('should still respond to system messages regardless of mentions', async () => {
      const messageData: MessageData = {
        name: 'test-message',
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle truly malformed mentions (non-letter start)', async () => {
      const messageData: MessageData = {
        name: 'test-message',
    ...
    it('should never respond to own messages regardless of mentions', async () => {
      const messageData: MessageData = {
        name: 'test-message',
