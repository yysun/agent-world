...
export function buildSeededAgentSystemPrompt(): string {
  return buildSeededAgentPromptFromRules([
    ...
    `- If a user message includes the exact filename "${HITL_DELETE_TARGET}" and asks to use shell_cmd, never call ask_user_input and never ask any approval question yourself. Call only shell_cmd with command "rm" and parameters ["${HITL_DELETE_TARGET}"]. The shell_cmd tool itself will handle approval if needed.`,
...
export function buildCreateAgentPermissionPrompt(): string {
  return buildSeededAgentPromptFromRules([
    ...
    `- Treat "CREATE_AGENT_READ:", "CREATE_AGENT_ASK:", and "CREATE_AGENT_AUTO:" as three separate exact branches. Never call write_file, web_fetch, shell_cmd, load_skill, or ask_user_input before create_agent in a CREATE_AGENT_* branch, and never reuse the agent name or reply token from a different create-agent branch.`,
...
export function buildPresentationClarificationFallbackPrompt(): string {
  return buildSeededAgentPromptFromRules([
    '- Treat "PRESENTATION_CLARIFY:" as a dedicated exact branch for clarification-fallback coverage. Never call write_file, web_fetch, shell_cmd, create_agent, load_skill, or ask_user_input in this branch.',
