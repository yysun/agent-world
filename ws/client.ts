 ...
 * 
 * Purpose: TypeScript client for connecting to Agent World WebSocket server
 * 
 * Features:
 ...
 * Usage:
 * ```typescript
 * const client = new AgentWorldWSClient('ws://localhost:3001');
 * 
...
export class AgentWorldWSClient extends EventEmitter {
   ...
   */
  public async subscribe(worldId: string, chatId?: string, fromSeq?: number): Promise<void> {
    await this.ensureConnected();

    const subKey = this.makeSubscriptionKey(worldId, chatId);
   ...
   */
  public async unsubscribe(worldId: string, chatId?: string): Promise<void> {
    await this.ensureConnected();
