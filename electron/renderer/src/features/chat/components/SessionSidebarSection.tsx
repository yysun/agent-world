 ...
 *
 * Recent Changes:
 * - 2026-04-24: Added a pending HITL indicator that lights the leading session dot for chats waiting on unresolved approval prompts.
 * - 2026-04-19: Extracted session sidebar ownership out of `LeftSidebarPanel`.
 */

import { Input } from '../../../design-system/primitives';

export function getSessionIndicatorClass(hasPendingHitlPrompt: boolean, isSelected: boolean): string {
  if (hasPendingHitlPrompt) {
    return isSelected
      ? 'bg-amber-300 shadow-[0_0_0_2px_rgba(251,191,36,0.2)]'
      : 'bg-amber-400 shadow-[0_0_0_2px_rgba(251,191,36,0.12)]';
  }
      ...
      <div className="min-h-0 flex-1 space-y-1 overflow-auto pr-1" data-testid="session-list">
        {sessions.length === 0 ? (
          ...
          filteredSessions.map((session) => {
            ...
            const hasPendingHitlPrompt = Boolean(session?.hasPendingHitlPrompt)
              || pendingHitlSessionIds.includes(session.id);
            ...
            return (
              <div
                ...
                className={`group w-full rounded-md py-1 pl-2 pr-0 text-left text-xs ${isSelected
              ...
              >
                <div className="flex items-center justify-between gap-1">
                  <div className="min-w-0 flex items-center gap-1.5">
                    <span
                      className={`h-1.5 w-1.5 shrink-0 rounded-full transition-colors ${getSessionIndicatorClass(hasPendingHitlPrompt, isSelected)}`}
                      aria-hidden="true"
                    />
