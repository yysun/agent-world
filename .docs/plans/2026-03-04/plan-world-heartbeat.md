...
**Date:** 2026-03-04  
**Last Updated:** 2026-03-04 (AR + SS progress synced)  
...
- [x] Add `node-cron` to root `package.json`.
- [x] Add `@types/node-cron` to root `devDependencies` (if needed by TS usage).
- [x] Add `node-cron` to `core/package.json`.

---

## Phase 2 - Core Types

- [x] Update `core/types.ts`:
...
- [x] Create `migrations/0016_add_world_heartbeat.sql`:
...
- [x] Update storage tests in `tests/core/storage/sqlite-storage.test.ts` for persistence/readback of new fields.

---

## Phase 4 - Core Heartbeat Scheduling Module

- [x] Add `core/heartbeat.ts` with:
...
- [x] Add focused unit tests under `tests/core/` for:
...
- [x] Add `electron/main-process/heartbeat-manager.ts`.
- [x] Maintain per-world job registry with status (`running` | `paused` | `stopped`).
- [x] Store resumable scheduler task handles (not only stop callbacks).
- [x] Provide manager API:
  - `startJob(world)`
  - `restartJob(world)`
  - `pauseJob(worldId)`
  - `resumeJob(worldId)`
  ...
  - `stopAll()`
  - `listJobs()`
- [x] Manager must deduplicate world jobs and guard invalid configs.
- [x] Serialize per-world manager operations to avoid restart/delete/tick races.

---

## Phase 6 - Main-Process Runtime Integration

- [x] In `electron/main-process/ipc-handlers.ts`:
...
- [x] Startup helper must be idempotent for repeated IPC/runtime entrypoints.
- [x] In workspace/runtime reset path, call `heartbeatManager.stopAll()` so workspace switches do not leak jobs.

---

## Phase 7 - IPC Contracts + Routes

- [x] Extend `electron/shared/ipc-contracts.ts`:
  - Add channels:
...
- [x] Wire channels in `electron/main-process/ipc-routes.ts` and `MainIpcHandlers` interface.
- [x] Implement corresponding handlers in `ipc-handlers.ts`.

---

## Phase 8 - Preload Bridge

- [x] Extend `electron/preload/bridge.ts` and `DesktopApi` contract:
  - `listHeartbeatJobs()`
  - `runHeartbeat(worldId)`
  - `pauseHeartbeat(worldId)`
  - `stopHeartbeat(worldId)`
- [x] Keep payload shape consistent with existing route helpers and invoke wrappers.

---

## Phase 9 - Renderer Integration

- [x] Update `electron/renderer/src/utils/app-helpers.ts`:
  - Add heartbeat fields to `getDefaultWorldForm()`.
  - Populate heartbeat fields in `getWorldFormFromWorld()`.
- [x] Update `electron/main-process/message-serialization.ts` (`serializeWorldInfo`) to include heartbeat fields so renderer receives and preserves values.
- [x] Update `electron/renderer/src/utils/validation.ts`:
  - If heartbeat enabled: require prompt and validate strict 5-field cron expression format.
- [x] Update `electron/renderer/src/components/RightPanelContent.tsx`:
...
- [x] Update `electron/renderer/src/App.tsx` unsaved-change detection to include heartbeat fields.
...
- [x] Add/update targeted unit tests (1-3 per affected boundary), including:
...
- [ ] Run `npm run test`.
- [x] Run `npm run integration` (required for API/runtime path changes).
